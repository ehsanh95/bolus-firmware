...
uint8_t tmp117_buf[3];
/* private (static) functions ----------------------- */
static int16_t  TMP117_twosCompToInt(uint16_t two_complement_val);							// convert a two's complement value to its integer form
static uint16_t TMP117_intToTwosComp(int16_t int_val);										// convert an integer value to its two's complement form
static bool 	TMP117_readReg2B(I2C_HandleTypeDef* i2c, uint8_t buffer[],					// get a 16-bit register value
												uint8_t reg_addr, uint16_t* reg_value);
static bool 	TMP117_writeReg2B(I2C_HandleTypeDef* i2c, uint8_t buffer[],					// write a 16-bit register value
								uint8_t reg_addr, uint16_t reg_value);
bool 		TMP117_setConfig(I2C_HandleTypeDef* i2c, uint8_t buffer[],					// set configuration register
											uint16_t reg_value);
static bool 	TMP117_getTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[],				// get temperature (result or offset)
													 uint8_t reg_addr, double* result);
static bool 	TMP117_setOffsetTemperature(I2C_HandleTypeDef* i2c,							// set offset temperature
																 uint8_t buffer[], double offset);
static bool 	TMP117_lockEEPROM(I2C_HandleTypeDef* i2c, uint8_t buffer[]);				// lock EEPROM
static bool 	TMP117_unlockEEPROM(I2C_HandleTypeDef* i2c, uint8_t buffer[]);				// unlock EEPROM
static bool 	TMP117_isEEPROMbusy(I2C_HandleTypeDef* i2c, uint8_t buffer[]);				// check whether EEPROM is busy
...
bool TMP117_Init(I2C_HandleTypeDef* i2c, uint8_t buffer[])
{
    ...
    // 1. Set operating mode (Continuous / One-shot / Shutdown)
    if (!TMP117_setConversionMode(i2c, buffer, TMP117_DEFAULT_MODE))
    {
        return false;
    }

    // 2. Set conversion cycle time
    if (!TMP117_setConversionTime(i2c, buffer, TMP117_CONVERSION_CYCLE))
    {
        return false;
    }

    // 3. Configure ALERT pin polarity
    if (!TMP117_setAlertPolarity(i2c, buffer, TMP117_ALERT_POLARITY))
    {
        return false;
    }

    // 4. Set averaging mode (for noise reduction)
    if (!TMP117_setAveraging(i2c, buffer, TMP117_DEFAULT_AVG_MODE))
    {
        return false;
    }

    // 5. Configure ALERT mode (Alert / Thermal / Data Ready)
    if (!TMP117_setAlertMode(i2c, buffer, TMP117_DEFAULT_ALERT_MODE))
    {
        return false;
    }

    // 6. Apply temperature offset correction if enabled
#if TMP117_USE_OFFSET
    if (!TMP117_setOffsetTemperature(i2c, buffer, TMP117_OFFSET_CORRECTION))
    {
        return false;
    }
#endif

    // 7. Set high temperature limit if enabled
#if TMP117_ALERT_HIGH_ENABLE
    if (!TMP117_setHighLimitTemperature(i2c, buffer, TMP117_HIGH_LIMIT_C))
    {
        return false;
    }
#endif

    // 8. Set low temperature limit if enabled
#if TMP117_ALERT_LOW_ENABLE
    if (!TMP117_setLowLimitTemperature(i2c, buffer, TMP117_LOW_LIMIT_C))
    {
        return false;
    }
#endif

    // 9. Allow time for sensor to stabilize after configuration
    HAL_Delay(TMP117_INIT_DELAY_MS);
...
{
	return TMP117_getTemperature(i2c, buffer, TMP117_TEMP_RES_REG, temp_c);
}
...
{
	return TMP117_readReg2B(i2c, buffer, TMP117_DEVICE_ID_REG, id);
}
...
{
	return TMP117_readReg2B(i2c, buffer, TMP117_CONFIG_REG, config);
}
 ...
 * @return	'true' if the data exchange was successful, otherwise - 'false'
 */
bool TMP117_setConfig(I2C_HandleTypeDef* i2c, uint8_t buffer[], uint16_t config)
{
	return TMP117_writeReg2B(i2c, buffer, TMP117_CONFIG_REG, config);
}
...
bool TMP117_setAveraging(I2C_HandleTypeDef* i2c, uint8_t buffer[], TMP117_AVG_MODE avg)
{
	...
	return TMP117_setConfig(i2c, buffer, reg_config_val);
}
 ...
 * @return	'true' if the data exchange was successful, otherwise - 'false'
 */
static bool TMP117_setOffsetTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[], double offset) {
	// convert to two's complement
	int16_t offset_int = round(offset / TMP117_RESOLUTION);
	uint16_t offset_two_comp = TMP117_intToTwosComp(offset_int);

	// write to the offset temperature register
	return TMP117_writeReg2B(i2c, buffer, TMP117_TEMP_OFFSET_REG, offset_two_comp);
}
...
bool TMP117_getOffsetTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[], double* offset) {
	return TMP117_getTemperature(i2c, buffer, TMP117_TEMP_OFFSET_REG, offset);
}
...
bool TMP117_setHighLimitTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[], double temp) {
	...
	uint16_t temp_two_comp = TMP117_intToTwosComp(temp_int);

	// write to the offset temperature register
	return TMP117_writeReg2B(i2c, buffer, TMP117_THIGH_LIM_REG, temp_two_comp);
}
...
bool TMP117_setLowLimitTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[], double temp) {
	...
	// write to the offset temperature register
	return TMP117_writeReg2B(i2c, buffer, TMP117_TLOW_LIM_REG, temp_two_comp);
}
...
bool TMP117_getHighLimitTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[], double* temp) {
	return TMP117_getTemperature(i2c, buffer, TMP117_THIGH_LIM_REG, temp);
}
...
bool TMP117_getLowLimitTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[], double* temp) {
	return TMP117_getTemperature(i2c, buffer, TMP117_TLOW_LIM_REG, temp);
}
...
bool TMP117_calibrate(I2C_HandleTypeDef* i2c, uint8_t buffer[], double target_temp) {
	...
	return TMP117_setOffsetTemperature(i2c, buffer, delta_temp);
}
...
bool TMP117_readEEPROM (I2C_HandleTypeDef* i2c, uint8_t buffer[], uint8_t eeprom_num, uint16_t* data) {
	if ((eeprom_num < 1) || (eeprom_num > 3)) return false;
		...
		if (!TMP117_isEEPROMbusy(i2c, buffer)) {
			// attempt to read a EEPROM
			switch (eeprom_num) {
				case 1:
					read_status = TMP117_readReg2B(i2c, buffer, TMP117_EEPROM1_REG, data);
					break;
				case 2:
					read_status = TMP117_readReg2B(i2c, buffer, TMP117_EEPROM2_REG, data);
					break;
				case 3:
					read_status = TMP117_readReg2B(i2c, buffer, TMP117_EEPROM3_REG, data);
					break;
				default:
					read_status = false;
					break;
...
bool TMP117_writeEEPROM (I2C_HandleTypeDef* i2c, uint8_t buffer[], uint16_t data, uint8_t eeprom_num) {
	...
	if (!TMP117_isEEPROMbusy(i2c, buffer)) {
		// unlock EEPROM
		write_status = TMP117_unlockEEPROM(i2c, buffer);
		// if EEPROM unlocked
		if (write_status) {
			// attempt to write a EEPROM
			switch (eeprom_num) {
				case 1:
					write_status = TMP117_writeReg2B(i2c, buffer,	TMP117_EEPROM1_REG, data);
					HAL_Delay(7);	// wait 7 ms (see datasheet, p.19)
					break;
				case 2:
					write_status = TMP117_writeReg2B(i2c, buffer,	TMP117_EEPROM2_REG, data);
					HAL_Delay(7);
					break;
				case 3:
					write_status = TMP117_writeReg2B(i2c, buffer,	TMP117_EEPROM3_REG, data);
					HAL_Delay(7);
					break;
				default:
					write_status = false;
			...
			uint32_t eeprom_wait_start = HAL_GetTick();
			while (TMP117_isEEPROMbusy(i2c, buffer)) {
				if ((HAL_GetTick() - eeprom_wait_start) >= TMP117_I2C_TIMEOUT_MS) {
					return false;
				}
			}

			// lock EEPROM
			TMP117_lockEEPROM(i2c, buffer);
		}
	}
	return write_status;
}
 ...
 */
static int16_t TMP117_twosCompToInt(uint16_t two_complement_val)
{
 ...
 */
static uint16_t TMP117_intToTwosComp(int16_t int_val) {
	// if positive
...
* @return	'true' if the data exchange was successful, otherwise - 'false'
*/
static bool TMP117_readReg2B(I2C_HandleTypeDef* i2c, uint8_t buffer[], uint8_t reg_addr, uint16_t* reg_value)
{
	...
	ret = HAL_I2C_Master_Transmit(i2c, TMP117_I2C_W_ADDR_GND,		// blocking function
																buffer, 1, TMP117_I2C_TIMEOUT_MS);
	if (ret != HAL_OK) {
		buffer[0] = 'E';
		buffer[1] = 'T';
		buffer[2] = 'X';
	...
	ret = HAL_I2C_Master_Receive(i2c, TMP117_I2C_R_ADDR_GND,
															 buffer, 2, TMP117_I2C_TIMEOUT_MS);
	if (ret != HAL_OK) {
		buffer[0] = 'E';
		buffer[1] = 'R';
		buffer[2] = 'X';
...
* @return	'true' if the data exchange was successful, otherwise - 'false'
*/
static bool TMP117_writeReg2B(I2C_HandleTypeDef* i2c, uint8_t buffer[], uint8_t reg_addr, uint16_t reg_value)
{
	HAL_StatusTypeDef ret;						// return status of i2c
	...
	ret = HAL_I2C_Master_Transmit(i2c, TMP117_I2C_W_ADDR_GND,		// blocking function
																buffer, 3, TMP117_I2C_TIMEOUT_MS);
	if (ret != HAL_OK) {
		buffer[0] = 'E';
		buffer[1] = 'T';
		buffer[2] = 'X';
...
* @return	'true' if the data exchange was successful, otherwise - 'false'
*/
static bool TMP117_getTemperature(I2C_HandleTypeDef* i2c, uint8_t buffer[], uint8_t reg_addr, double* result) {
	uint16_t twos_comp_temp_val = 0;
	if (!TMP117_readReg2B(i2c, buffer, reg_addr, &twos_comp_temp_val)) return false;

	// convert to double temperature value (Celsius)
	int16_t dec_temp_val = TMP117_twosCompToInt(twos_comp_temp_val);
...
* @return	'true' if the data exchange was successful, otherwise - 'false'
*/
static bool TMP117_lockEEPROM(I2C_HandleTypeDef* i2c, uint8_t buffer[]) {
	// reset bit 15 (EUN) of the EEPROM Unlock Register
	uint16_t code = 0;
	code &= ~(1UL << 15);
	return TMP117_writeReg2B(i2c, buffer, TMP117_EEPROM_UL_REG, code);
}
...
* @return	'true' if the data exchange was successful, otherwise - 'false'
*/
static bool TMP117_unlockEEPROM(I2C_HandleTypeDef* i2c, uint8_t buffer[]) {
...
*/
static bool TMP117_isEEPROMbusy(I2C_HandleTypeDef* i2c, uint8_t buffer[]) {
	uint16_t code = 0;
	if (!TMP117_readReg2B(i2c, buffer,	TMP117_EEPROM_UL_REG, &code)) return false;
