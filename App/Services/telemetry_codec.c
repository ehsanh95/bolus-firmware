    ...
    size_t *payload_size)
{
    ...
    *payload_size = 0U;
    ...
    payload[0] = (uint8_t)(
        ((BOLUS_TELEMETRY_PROTOCOL_VERSION & 0x0FU) << 4) |
    ...
    if (summary->temperature_valid)
    {
        WriteI16Le(
            &payload[8],
            MdegCToCentiC(summary->temperature_current_mdeg_c));
        WriteI16Le(
            &payload[10],
            MdegCToCentiC(summary->temperature_min_mdeg_c));
        WriteI16Le(
            &payload[12],
            MdegCToCentiC(summary->temperature_max_mdeg_c));
        WriteI16Le(
            &payload[14],
            MdegCToCentiC(summary->max_negative_excursion_mdeg_c));
    }
